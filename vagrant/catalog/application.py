...
def gconnect():
    ...
    try:
        ...
        response = make_response(json.dumps(
            'Failed to upgrade the authorization code for token'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    if result['issued_to'] != CLIENT_ID:
        ...
        response = make_response(json.dumps(
            "Token's client ID does not match app"), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    output = '<h1>Welcome, %s!</h1><img src="%s" \
    styles="width: 300px; height: 300px;border-radius: 150px; \
...
def gdisconnect():
    ...
    try:
        ...
        response = make_response(json.dumps(
            'Unable to remove user session'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def main():
    ...
    return render_template(
        'main.html', state=generateState(),
        isLogin=login_session['logIn'], teams=getAllTeams())
...
def addPlayer():
    if 'user_id' in login_session:
        if request.method == 'GET':
            return render_template(
                'addPlayer.html', isLogin=login_session['logIn'],
                teams=getAllTeams())
        if request.method == 'POST':
            ...
            flash("New Player added: %s for team:%s" %
                  (request.form['player_name'], team_name))
            return redirect(url_for('main'))
    else:
        return redirect(url_for('main'))
...
def team(team_id):
    ...
    if 'user_id' not in login_session:
        return render_template(
            'team.html', state=generateState(),
            isLogin=False, team=teamName,
            teams=getAllTeams(), players=players)

    return render_template(
        'team.html', state=generateState(),
        isLogin=login_session['logIn'], team=teamName,
        teams=getAllTeams(), players=players)
...
def player(player_id):
    if 'user_id' not in login_session:
        return render_template(
            'player.html', state=generateState(),
            isLogin=False, player=getPlayer(player_id))
    ...
    return render_template(
        'player.html', state=generateState(),
        session=login_session, player=getPlayer(player_id))
...
def editPlayer(player_id):
    ...
    if request.method == 'GET':
        ...
        return render_template(
            'editPlayer.html', player=player, teams=getAllTeams())
        ...
        flash("Player Edited: %s for team:%s" %
              (request.form['player_name'], team_name))
        return redirect(url_for('main'))
